...
class MusicRepository(private val musicDao: MusicDao) {
    ...
    val allSongsWithArtists: Flow<List<SongWithArtists>> = musicDao.getSongsWithArtists()

    fun getArtistWithSongs(artistId: String): Flow<ArtistWithSongs> {
        return musicDao.getArtistWithSongs(artistId)
    }

    fun getAlbumWithSongs(albumId: String) = musicDao.getAlbumWithSongs(albumId)
